...
const interacaoSchema = Joi.object({
  ...
  prox_acao: Joi.string().allow('', null)
});

export async function listInteracoes() {
  const [rows] = await db.execute(
    `SELECT i.id, c.nome AS cliente, o.nome AS oportunidade, i.tipo, i.descricao, i.prox_acao, DATE_FORMAT(i.data, '%d/%m/%Y') AS data
     FROM interacoes i
...
export async function createInteracao(payload) {
  ...
  await db.execute(
    'INSERT INTO interacoes (cliente_id, oportunidade_id, tipo, descricao, prox_acao, data, criado_em) VALUES (?, ?, ?, ?, ?, NOW(), NOW())',
    [value.cliente_id, oportunidadeId, value.tipo, value.descricao || '', value.prox_acao || '']
  );
}
